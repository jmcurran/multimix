...
mStep <- function(Z, D, P) {
  ...
  for (i in seq_along(D$dlink)) P$dstat[[i]] <- crossprod(P$W, D$dvals[[i]])

  for (i in seq_along(D$lcdisc)) P$ldstat[[i]] <- crossprod(P$W, D$ldvals[[i]])
  ...
  for (i in seq_along(D$cdep)) {
    ...
    for (j in seq_len(D$numClusters)) {
      for (ii in seq_len(nxp)) {
        P$MVMV[[i]][[j]][left(ii), right(ii)] <- P$MVMV[[i]][[j]][right(ii), left(ii)] <- ccov[[i]][j,
          ii]
      }
    }
  }
  ...
  for (i in seq_along(D$lcdep)) {
    ...
    for (j in seq_len(D$numClusters)) {
      ...
      for (lv in seq_len(D$ldlevs[i])) {
        for (ii in seq_len(nxp)) {
          lccov[[i]][[lv]][, ii] <- (P$lcpstat[[i]][[lv]][, ii] - P$lcstat[[i]][[lv]][, left(ii)] * P$lcstat[[i]][[lv]][,
          right(ii)])
          M[right(ii), left(ii)] <- M[right(ii), left(ii)] + lccov[[i]][[lv]][j, ii] * P$ldstat[[i]][j,
          lv]
          M[left(ii), right(ii)] <- M[right(ii), left(ii)]
        }  #ii
      }  #lv
